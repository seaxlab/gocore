...
import (
	"github.com/seaxlab/gocore/src/lang/treex"
	"hash/fnv"
	"strconv"
)
...
func NewHashTable() *HashTable {
	...
	var hashtable []*treex.RBTree
	for i := 0; i < M; i++ {
		hashtable = append(hashtable, treex.NewRBTree())
	}
...
func (h *HashTable) resize(newM int) {
	var newHashTable []*treex.RBTree
	for i := 0; i < newM; i++ {
		newHashTable = append(newHashTable, treex.NewRBTree())
	}
	oldM := h.M
	h.M = newM
	for i := 0; i < oldM; i++ {
