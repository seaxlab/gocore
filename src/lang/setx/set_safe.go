...
// NewSafeSet creates and returns a reference to an empty set.
// Operations on the resulting set are thread-safe.
func NewSafeSet(s ...interface{}) Set {
	set := newSafeSet()
	for _, item := range s {
		set.Add(item)
	}
...
// NewSafeSetWith creates and returns a new set with the given elements.
// Operations on the resulting set are thread-safe.
func NewSafeSetWith(elts ...interface{}) Set {
	return NewSafeSetFromSlice(elts)
}

// NewSafeSetFromSlice creates and returns a reference to a set from an
// existing slice.  Operations on the resulting set are thread-safe.
func NewSafeSetFromSlice(s []interface{}) Set {
	a := NewSafeSet(s...)
	return a
}
