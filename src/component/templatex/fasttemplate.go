...
import (
	"bytes"
	"fmt"
	"github.com/seaxlab/gocore/src/component/bytebufferpool"
	"github.com/seaxlab/gocore/src/lang/unsafex"
	"io"
)
...
func ExecuteFunc(template, startTag, endTag string, w io.Writer, f TagFunc) (int64, error) {
	s := unsafex.String2Bytes(template)
	a := unsafex.String2Bytes(startTag)
	b := unsafex.String2Bytes(endTag)
	...
	for {
		...
		ni, err = f(w, unsafex.Bytes2String(s[:n]))
		nn += int64(ni)
		if err != nil {
			return nn, err
		}
...
func ExecuteFuncStringWithErr(template, startTag, endTag string, f TagFunc) (string, error) {
	tagsCount := bytes.Count(unsafex.String2Bytes(template), unsafex.String2Bytes(startTag))
	if tagsCount == 0 {
		return template, nil
	}
...
func (t *Template) Reset(template, startTag, endTag string) error {
	...
	for {
		...
		t.tags = append(t.tags, unsafex.Bytes2String(s[:n]))
		s = s[n+len(b):]
	}
...
func (t *Template) ExecuteFunc(w io.Writer, f TagFunc) (int64, error) {
	...
	n := len(t.texts) - 1
	if n == -1 {
		ni, err := w.Write(unsafex.String2Bytes(t.template))
		return int64(ni), err
	}
...
func keepUnknownTagFunc(w io.Writer, startTag, endTag, tag string, m map[string]interface{}) (int, error) {
	v, ok := m[tag]
	if !ok {
		if _, err := w.Write(unsafex.String2Bytes(startTag)); err != nil {
			return 0, err
		}
		if _, err := w.Write(unsafex.String2Bytes(tag)); err != nil {
			return 0, err
		}
		if _, err := w.Write(unsafex.String2Bytes(endTag)); err != nil {
			return 0, err
		}
