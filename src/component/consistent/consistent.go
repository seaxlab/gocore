...
func New() *Consistent {
	c := new(Consistent)
	c.NumberOfReplicas = 32
	c.circle = make(map[uint32]string)
	c.members = make(map[string]bool)
	return c
}

// nodeKey generates a string key for an element with an index.
func (c *Consistent) nodeKey(node string, idx int) string {
...
func (c *Consistent) add(node string) {
	for i := 0; i < c.NumberOfReplicas; i++ {
		c.circle[c.hashKey(c.nodeKey(node, i))] = node
	}
	c.members[node] = true
...
func (c *Consistent) remove(node string) {
	for i := 0; i < c.NumberOfReplicas; i++ {
		delete(c.circle, c.hashKey(c.nodeKey(node, i)))
	}
...
func (c *Consistent) Set(nodes []string) {
	c.Lock()
	defer c.Unlock()
...
func (c *Consistent) Rebuild(nodes []string) {
	c.Set(nodes)
}
